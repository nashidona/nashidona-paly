...
export default function Home() {
  ...
  const [queue, setQueue] = useState<Track[]>(() => {
    try { const raw = localStorage.getItem('nd_queue'); return raw? JSON.parse(raw): []; } catch { return []; }
  });
  const [current, setCurrent] = useState<Track | null>(() => {
    try {
      const raw = localStorage.getItem('nd_current');
      const id = raw? JSON.parse(raw): null;
      const qraw = localStorage.getItem('nd_queue'); const arr: Track[] = qraw? JSON.parse(qraw): [];
      return id? (arr.find(x => String(x.id) === String(id)) || null) : null;
    } catch { return null; }
  });
  ...
  function move(id: Track['id'], dir: -1|1) {
    setQueue(q => { const i = q.findIndex(x => String(x.id) === String(id)); if (i < 0) return q;
      const j = i + dir; if (j < 0 || j >= q.length) return q; const c=[...q]; const tmp=c[i]; c[i]=c[j]; c[j]=tmp; return c; });
  }
  function playNext(autoplay = false) {
    setQueue(q => { if (!q.length) { setCurrent(null); return q; }
      const idx = current ? q.findIndex(x => String(x.id) === String(current.id)) : -1;
      const next = (idx >= 0 && idx < q.length - 1) ? q[idx + 1] : q[0];
      setCurrent(next); if (autoplay) autoPlayPending.current = true; return q; });
  }
  function playPrev(autoplay = false) {
    setQueue(q => { if (!q.length) { setCurrent(null); return q; }
      const idx = current ? q.findIndex(x => String(x.id) === String(current.id)) : -1;
      const prev = (idx > 0) ? q[idx - 1] : q[q.length - 1];
      setCurrent(prev); if (autoplay) autoPlayPending.current = true; return q; });
  }
  ...
  return (<div dir='rtl' style={{fontFamily:'system-ui,-apple-system,Segoe UI,Tahoma',background:'#f8fafc',minHeight:'100vh'}}>
    <header style={{position:'sticky',top:0,background:'#fff',borderBottom:'1px solid #e5e7eb',zIndex:10}}>
      <div style={{maxWidth:960,margin:'0 auto',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        ...
        <div style={{fontSize:12,color:'#6b7280'}}>النتائج: {items.length} / {count}</div>
      </div>
    </header>
    ...
    <section style={{maxWidth:960,margin:'20px auto 12px auto',padding:'12px 16px'}}>
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:12}}>
        ...
        <div style={{fontSize:12,color:'#047857'}}>يبدأ البحث تلقائيًا أثناء الكتابة · لا حاجة للزر</div>
      </div>
    </section>

    <main style={{maxWidth:960,margin:'0 auto',padding:'0 16px 120px 16px'}}>
      <div style={{display:'grid',gap:12}}>
        {items.map(tr=>(<div key={String(tr.id)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',border:'1px solid #e5e7eb',borderRadius:12,padding:12,background:'#fff'}}>
          ...
          <div style={{display:'flex',gap:8}}>
            <button onClick={()=>addToQueue(tr)} style={{padding:'6px 8px',border:'1px solid #d1fae5',borderRadius:8}}>+ قائمة</button>
            <button onClick={()=>{playNow(tr);}} style={{padding:'6px 8px',background:'#059669',color:'#fff',borderRadius:8}}>▶ تشغيل</button>
          </div>
        </div>))}
      </div>
      <div style={{display:'flex',justifyContent:'center',padding:'16px 0'}}>
        {canLoadMore && (<button disabled={loading} onClick={()=>{ const next=offset+60; setOffset(next); fetchPage(next,true); }} style={{padding:'10px 14px',border:'1px solid #d1fae5',borderRadius:10}}>{loading? '...جارٍ التحميل':'عرض المزيد'}</button>)}
      </div>
    </main>
          ...
          <div style={{display:'grid',gap:8,marginTop:8,maxHeight:240,overflowY:'auto'}}>
            {queue.map((tr,i)=>(<div key={String(tr.id)} style={{display:'flex',justifyContent:'space-between',alignItems:'center',border:'1px solid #e5e7eb',borderRadius:10,padding:'6px 8px',background: current&&String(current.id)===String(tr.id)? '#ecfdf5':'#fff'}}>
              ...
              <div style={{display:'flex',gap:6}}>
                ...
                <button onClick={()=>{setCurrent(tr); autoPlayPending.current=true;}} title='تشغيل'>▶</button>
              </div>
            </div>))}
