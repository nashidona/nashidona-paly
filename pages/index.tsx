...
export default function Home() { const [lyricsMap, setLyricsMap] = useState<Record<string, boolean>>({}); const [q, setQ] = useState(''); const dq = useDebounced(q, 350);

const [items, setItems] = useState<Track[]>([]); const [count, setCount] = useState<number>(0); const [offset, setOffset] = useState(0); const [hasMore, setHasMore] = useState(true); const [loading, setLoading] = useState(false); const [err, setErr] = useState<string>(''); const [albumInfo, setAlbumInfo] = useState<string>('');

// UI + audio state const [open, setOpen] = useState(false); const [t, setT] = useState(0); const [dur, setDur] = useState(0); const [showLyrics, setShowLyrics] = useState<{open:boolean, title?:string, text?:string}>({open:false});

// Feedback UI const [fbOpen, setFbOpen] = useState(false); const [fbMsg, setFbMsg] = useState(''); const [fbEmail, setFbEmail] = useState(''); const [fbBusy, setFbBusy] = useState(false); const [fbOk, setFbOk] = useState<string>('');

// queue/player const [queue, setQueue] = useState<Track[]>([]); const [current, setCurrent] = useState<Track | null>(null); const [loop, setLoop] = useState<LoopMode>('queue'); const [sleepAt, setSleepAt] = useState<number|null>(null); const [hydrated, setHydrated] = useState(false); // لا نحفظ قبل القراءة

// ✅ دعم التشغيل عبر ?play=ID const [needsTap, setNeedsTap] = useState(false); const [incomingTrack, setIncomingTrack] = useState<Track | null>(null);

const audioRef = useRef<HTMLAudioElement | null>(null); const autoPlayPending = useRef(false); const sentinelRef = useRef<HTMLDivElement | null>(null); const footerRef = useRef<HTMLDivElement | null>(null); const loadingRef = useRef(false); loadingRef.current = loading;

// مراقبة التعليق/العطب const lastProgressRef = useRef<number>(0); const retryRef = useRef<number>(0); const watchdogRef = useRef<ReturnType<typeof setInterval> | null>(null); const MAX_RETRIES = 2; const STUCK_MS    = 15000; const CHECK_EVERY = 4000;

function startWatchdog(a: HTMLAudioElement | null) { stopWatchdog(); if (!a) return; lastProgressRef.current = Date.now(); watchdogRef.current = setInterval(() => { if (!a || a.paused) return; const since = Date.now() - lastProgressRef.current; if (since > STUCK_MS) { if (retryRef.current < MAX_RETRIES) { retryRef.current++; try { a.load(); a.play().catch(()=>{}); } catch {} lastProgressRef.current = Date.now(); } else { reportBad('stuck_no_progress', no progress for ${since}ms after ${retryRef.current} retries); playNext(true); } } }, CHECK_EVERY); } function stopWatchdog() { if (watchdogRef.current) { clearInterval(watchdogRef.current); watchdogRef.current = null; } }

function dedup(arr: Track[]) { const seen = new Set<string>(); return arr.filter(x => { const k = String(x.id); if (seen.has(k)) return false; seen.add(k); return true; }); }

async function fetchPage(newOffset = 0, append = false) { setLoading(true); setErr(''); try { const r = await fetch(/api/search?q=${encodeURIComponent(dq)}&limit=60&offset=${newOffset}); if (!r.ok) throw new Error(String(r.status)); const j = await r.json(); const page: Track[] = dedup(j.items || []); const total = typeof j.count === 'number' ? j.count : count; setCount(total); setHasMore((page.length === 60) || ((newOffset + page.length) < total)); setItems(prev => append ? dedup([...prev, ...page]) : page); } catch (e:any) { setErr('تعذر جلب النتائج الآن'); if (!append) setItems([]); setHasMore(false); } finally { setLoading(false); } }

// أول تحميل: عشوائي ثم ضبط العد الحقيقي للتمرير useEffect(() => { let cancelled = false; async function load() { setOffset(0); setHasMore(true); setErr('');
...
// ✅ تحضير توفر الكلمات لعناصر الصفحة الأولى فقط (خفيف) useEffect(() => { if (!items.length) return; const sample = items.slice(0, 36).filter(it => lyricsMap[String(it.id)] === undefined); if (!sample.length) return; (async () => { for (const it of sample) { try { const r = await fetch(/api/track?id=${it.id}); const j = await r.json(); const has = !!(j?.lyrics && String(j.lyrics).trim()); if (has) setLyricsMap(m => ({ ...m, [String(it.id)]: true })); } catch {} } })(); }, [items]);

// ✅ تشغيل تلقائي عند القدوم بـ ?play=ID useEffect(() => { if (typeof window === 'undefined') return; try { const params = new URLSearchParams(window.location.search); const pid = params.get('play'); if (pid && /^\d+$/.test(pid)) { fetch(/api/track?id=${pid}&full=1) .then(r => r.ok ? r.json() : Promise.reject()) .then(js => { const it = js?.item; if (!it) return; const tr: Track = { id: it.id, title: it.title, album: it.album || undefined, artist: it.artist || it.artist_text || undefined, artist_text: it.artist_text || undefined, class_parent: it.class_parent || undefined, class_child: it.class_child || undefined, cover_url: it.cover_url || undefined, year: it.year || undefined, has_lyrics: !!(js?.lyrics && String(js.lyrics).trim()) }; setIncomingTrack(tr); playNow(tr); const a = audioRef.current as HTMLAudioElement | null; if (a) { a.play().catch(() => setNeedsTap(true)); } else { setNeedsTap(true); } }) .catch(()=>{}); } } catch {} // eslint-disable-next-line react-hooks/exhaustive-deps }, []);

// تمرير لا نهائي useEffect(() => { const el = sentinelRef.current; if (!el) return;
...
// قفل التمرير عند فتح القائمة useEffect(() => { if (typeof document === 'undefined') return; const prev = document.body.style.overflow; document.body.style.overflow = open ? 'hidden' : prev || ''; return () => { document.body.style.overflow = prev; }; }, [open]);

// keyboard offset useEffect(() => { if (typeof window === 'undefined') return; const vv = (window as any).visualViewport; if (!vv) return; const onResize = () => { const diff = vv.height ? Math.round(window.innerHeight - vv.height) : 0; const kb = diff > 60 ? diff : 0; document.documentElement.style.setProperty('--kb', kb + 'px'); }; vv.addEventListener('resize', onResize); onResize(); return () => vv.removeEventListener('resize', onResize); }, []);

// footer height -> bottom padding useEffect(() => { if (typeof window === 'undefined') return; const measure = () => { const h = footerRef.current?.getBoundingClientRect().height || 140; document.documentElement.style.setProperty('--footerH', ${Math.round(h)}px); }; measure(); const ro = new ResizeObserver(measure); if (footerRef.current) ro.observe(footerRef.current); window.addEventListener('resize', measure); return () => { window.removeEventListener('resize', measure); ro.disconnect(); }; }, []);

// ====== استرجاع الحالة من التخزين المحلي ====== useEffect(() => { if (typeof window === 'undefined') return; try { const rawQueue = localStorage.getItem('nd_queue'); const qArr: Track[] = rawQueue ? JSON.parse(rawQueue) : []; setQueue(Array.isArray(qArr) ? qArr : []);
...
// ====== وظائف التشغيل ====== function playNow(tr: Track) { setCurrent(tr); setQueue(q => (q.find(x => String(x.id) === String(tr.id)) ? q : [tr, ...q])); autoPlayPending.current = true; retryRef.current = 0; setMediaSession({ ...tr, artist: tr.artist || tr.artist_text }, audioRef.current!); } function addToQueue(tr: Track) { setQueue(q => (q.find(x => String(x.id) === String(tr.id)) ? q : [...q, tr])); } function clearQueue() { setQueue([]); setCurrent(null); } function removeFromQueue(id: Track['id']) { setQueue(q => q.filter(x => String(x.id) !== String(id))); if (current && String(current.id) === String(id)) setTimeout(() => playNext(true), 0); } function move(id: Track['id'], dir: -1|1) { setQueue(q => { const i = q.findIndex(x => String(x.id) === String(id)); if (i < 0) return q; const j = i + dir; if (j < 0 || j >= q.length) return q; const c=[...q]; const tmp=c[i]; c[i]=c[j]; c[j]=tmp; return c; }); } function shuffleQueue() { setQueue(q => { const c=[...q]; for (let i=c.length-1;i>0;i--){ const j=Math.floor(Math.random()*(i+1)); [c[i],c[j]]=[c[j],c[i]];} return c; }); } function playNext(autoplay = false) { setQueue(q => { if (!q.length) { setCurrent(null); return q; } const idx = current ? q.findIndex(x => String(x.id) === String(current.id)) : -1; const next = (idx >= 0 && idx < q.length - 1) ? q[idx + 1] : q[0]; setCurrent(next); retryRef.current = 0; if (autoplay) autoPlayPending.current = true; setMediaSession({ ...next, artist: next.artist || next.artist_text }, audioRef.current!); return q; }); } function playPrev(autoplay = false) { setQueue(q => { if (!q.length) { setCurrent(null); return q; } const idx = current ? q.findIndex(x => String(x.id) === String(current.id)) : -1; const prev = (idx > 0) ? q[idx - 1] : q[q.length - 1]; setCurrent(prev); retryRef.current = 0; if (autoplay) autoPlayPending.current = true; setMediaSession({ ...prev, artist: prev.artist || prev.artist_text }, audioRef.current!); return q; }); } function seek(v: number) { const a = audioRef.current; if (!a) return; a.currentTime = v; setT(v); }

// مفاتيح القفل useEffect(()=>{ if (typeof window === 'undefined') return; (window as any).__playNext = ()=>playNext(true); (window as any).__playPrev = ()=>playPrev(true); }, [queue, current]);

// إرسال تقرير رابط معطوب async function reportBad(reason: string, detail?: string) { try { if (!current) return; await fetch('/api/report-bad-link', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ track_id: current.id, reason, detail: detail || '', retries: retryRef.current || 0, }) }); } catch {} }

// أحداث الصوت + مراقب التعليق useEffect(() => { const a = audioRef.current; if (!a) return;
...
// حفظ الحالة محليًا — بعد الهيدرايشن useEffect(() => { if (!hydrated || typeof window === 'undefined') return; try { localStorage.setItem('nd_queue', JSON.stringify(queue)); } catch {} }, [queue, hydrated]); useEffect(() => { if (!hydrated || typeof window === 'undefined') return; try { localStorage.setItem('nd_current', JSON.stringify(current?.id ?? null)); } catch {} }, [current, hydrated]); useEffect(() => { if (!hydrated || typeof window === 'undefined') return; try { localStorage.setItem('nd_loop', loop); } catch {} }, [loop, hydrated]); useEffect(() => { if (!hydrated || typeof window === 'undefined') return; try { localStorage.setItem('nd_sleep', JSON.stringify(sleepAt)); } catch {} }, [sleepAt, hydrated]);

function startSleep(minutes:number){ const when = Date.now() + minutes601000; setSleepAt(when); }

// إضافة كل النتائج إلى القائمة async function addAllResultsToQueue() { if (dq.trim() === '') { if (!items.length) return; setQueue(q => { const seen = new Set(q.map(x => String(x.id))); const merged = [...q]; items.forEach(tr => { const k = String(tr.id); if (!seen.has(k)) { merged.push(tr); seen.add(k); } }); return merged; }); alert(تمت إضافة ${items.length} إلى قائمة التشغيل); return; }
...
// جلب كلمات عند الطلب async function openLyrics(tr: Track) { try { const r = await fetch(/api/track?id=${tr.id}); const j = await r.json(); const errText = (j && j.error) ? String(j.error) : ''; const txt = (j?.lyrics || '').trim(); setShowLyrics({open:true, title: tr.title, text: errText ? errText : (txt || 'لا توجد كلمات متاحة.')}); } catch { setShowLyrics({open:true, title: tr.title, text: 'تعذر جلب الكلمات حالياً.'}); } }

// بانر معلومات الألبوم const singleAlbum = (() => { if (!items.length) return null; const uniq = Array.from(new Set(items.map(x => x.album || ''))); if (uniq.length === 1 && uniq[0]) { const sample = items[0]; return { title: uniq[0], year: sample.year || '', cover: sample.cover_url || '/logo.png' }; } return null; })();

// جلب info للألبوم useEffect(() => { let cancelled = false; if (singleAlbum?.title) { fetch(/api/album?title=${encodeURIComponent(singleAlbum.title)}) .then(r => r.json()) .then(j => { if (!cancelled) setAlbumInfo((j?.info || '').trim()); }) .catch(() => { if (!cancelled) setAlbumInfo(''); }); } else { setAlbumInfo(''); } return () => { cancelled = true; }; }, [singleAlbum?.title]);

// إرسال ملاحظة async function submitFeedback() { if (!fbMsg.trim()) { setFbOk('من فضلك اكتب ملاحظتك أولاً.'); return; } setFbBusy(true); setFbOk(''); try { const ua = typeof navigator!=='undefined' ? navigator.userAgent : ''; const page = typeof location!=='undefined' ? location.href : ''; const track_id = current ? current.id : null; const r = await fetch('/api/feedback', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ message: fbMsg.trim(), email: fbEmail.trim() || null, ua, page, track_id }) }); const j = await r.json(); if (j && j.ok) { setFbOk('✅ تم إرسال ملاحظتك، شكراً لك.'); setFbMsg(''); setFbEmail(''); setTimeout(()=>{ setFbOpen(false); setFbOk(''); }, 1200); } else { setFbOk('تم الاستلام محلياً، سنراجعها (قد لا يكون التخزين مفعلاً بعد).'); } } catch { setFbOk('تعذّر الإرسال الآن، حاول لاحقاً.'); } finally { setFbBusy(false); } }

return (<div style={{fontFamily:'system-ui,-apple-system,Segoe UI,Tahoma',background:'#f8fafc',minHeight:'100vh'}}> <header style={{position:'sticky',top:0,background:'#fff',borderBottom:'1px solid #e5e7eb',zIndex:10}}> <div style={{maxWidth:960,margin:'0 auto',padding:'10px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,flexWrap:'wrap'}}> <div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}> <img src='/logo.png' width={36} height={36} alt='logo'/><b>Nashidona • النسخة التجريبية</b> <button onClick={()=>setFbOpen(true)} className="fbBtn" title="أرسل ملاحظة">💬 ملاحظات</button> </div> <div className='stats' style={{fontSize:12,color:'#6b7280'}}>النتائج: {items.length}{count?  / ${count}:''}</div> </div> </header>
    ...
    {items.map(tr=>{
      ...
      <div key={String(tr.id)} className='trackCard'
        ...
        <div className='actions' style={{display:'flex',gap:8, alignItems:'center'}}>
          ...
          <button className='btn sm' title='مشاركة' onClick={() => {
            try {
